...
df_validation = df_validation.groupby(['date']).agg({'gallons': ['sum']}).reset_index()
df_validation.columns = df_validation.columns.droplevel()
df_validation.columns =['Date', 'Real_Energy']
df_validation['Date']=pd.to_datetime(df_validation['Date'])
...
def compute_model_performance(hybrid=False, electric=False):
    ...
    if hybrid==True:
        ...
        df_grouped = df_new.groupby(['Date']).agg({'Energy': ['sum'] ,'Powertrain': ['max'], 'dist': ['sum']}).reset_index()
        df_grouped.columns = df_grouped.columns.droplevel()
        df_grouped.columns =['Date', 'Energy', 'Powertrain', 'Distance']
        cols = ['Date']
        ...
        RMSE_Energy_current = mean_squared_error(test['Real_Energy'], test['Energy'], squared=False)
